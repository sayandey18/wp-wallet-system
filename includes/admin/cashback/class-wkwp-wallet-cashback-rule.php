...
class WKWP_Wallet_Cashback_Rule {
	...
	public function wkwp_wallet_edit_cashback_rule() {
		...
		if ( $rule_id > 0 ) {
			...
			$default_data['rule_annually']   = empty( $get_data['rule_annually'] ) ? 0 : $get_data['rule_annually'];
		...
		<div class="wrap wkwp-wallet-edit-cashback">
			...
			<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
				<table class="form-table">
					...
					<tr valign="top">
						<th scope="row" class="titledesc">
							<label for="mp_advanced_cashback_rule_type"><?php esc_html_e( 'Cashback Type', 'wp-wallet-system' ); ?></label>
						</th>

						<td class="">
							<select class="" name="rule_type" id="mp_advanced_cashback_rule_type" title="cashback type">
					...
					<tr valign="top">
						<th scope="row" class="titledesc">
							<label for="mp_advanced_price_from"><?php esc_html_e( 'Cart Total From', 'wp-wallet-system' ); ?>
							<span class="error"> *</span></label>
						</th>

						<td>
							<input step="0.01" type="number" name="rule_price_from" id="mp_advanced_price_from" value="<?php echo esc_attr( $rule_data['rule_price_from'] ); ?>" min="1" required>
						</td>
					</tr>

					<tr valign="top">
						<th scope="row" class="titledesc">
							<label for="mp_advanced_price_to"><?php esc_html_e( 'Cart Total To', 'wp-wallet-system' ); ?>
							<span class="error"> *</span></label>
						</th>

						<td>
							<input step="0.01" type="number" name="rule_price_to" id="mp_advanced_price_to" value="<?php echo esc_attr( $rule_data['rule_price_to'] ); ?>" min="1" required>
						</td>
					</tr>

					<tr valign="top">
						<th scope="row" class="titledesc">
							<label for="mp_advanced_annually"><?php esc_html_e( 'Yearly Purchase', 'wp-wallet-system' ); ?>
						</th>

						<td>
							<input type="checkbox" name="rule_annually" id="mp_advanced_annually" value="1" <?php checked( $rule_data['rule_annually'], 1 ); ?>>
						</td>
					</tr>
	...
	public function wkwp_wallet_handle_cb_rule_form( $post_data ) {
		...
		$has_error = $this->wkwp_wallet_validate_form( $post_data, $rule_id );
		...
		$rule_data = array(
			...
			'rule_annually'   => empty( $post_data['rule_annually'] ) ? 0 : $post_data['rule_annually'],
	 ...
	 * @return string
	 */
	public function wkwp_wallet_validate_form( $form_data = array(), $rule_id ) {
