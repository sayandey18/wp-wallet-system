...
if ( ! class_exists( 'WKWP_Wallet_WK_Caching_Core' ) ) {
	/**
	 * WKWP_Wallet_WK_Caching_Core
	 */
	class WKWP_Wallet_WK_Caching_Core {
		/**
		 * WP Wallet Loading WC Wallet Version.
		 *
		 * @var $version
		 */
		public static $version = WKWP_WALLET_WK_CACHING_VERSION;
		...
		public static function load_files() {
			$get_global_path = WKWP_WALLET_PLUGIN_FILE . 'wk_caching/';

			if ( false === @file_exists( $get_global_path . '/includes/class-wk-caching.php' ) ) {
				_doing_it_wrong( __FUNCTION__, esc_html__( 'WK Caching Core should be present in folder \'wk_caching\includes\' in order to run this properly.', 'wp-wallet-system' ), esc_html( self::$version ) );
			...
			if ( WK_CACHING_VERSION === self::$version ) {
				...
				_doing_it_wrong( __FUNCTION__, esc_html__( 'WK Caching Core should be at the same version as declared in your "class-wk-caching-core-loader.php"', 'wp-wallet-system' ), esc_html( self::$version ) );
				if ( defined( 'WKWC_DEV' ) && true === WKWC_DEV ) {
					die( 0 );
				}
				return false;
	...
	WKWP_Wallet_WK_Caching_Core::register();
}
