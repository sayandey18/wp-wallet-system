	...
	class WK_Caching_Transient {
		...
		public function delete_all_transients( $transient_group = '' ) {
			...
			if ( $transient_caching_enabled ) {
				...
				$deleted = $wpdb->query( $query ); //phpcs:ignore WordPress.DB.PreparedSQL
		...
		protected function is_phpfast_caching_enabled() {
			...
			if ( ! empty( $configuation['plugin_path'] ) && file_exists( $configuation['plugin_path'] . 'wk_caching/vendor/autoload.php' ) ) {
				$enabled = true;
			}
